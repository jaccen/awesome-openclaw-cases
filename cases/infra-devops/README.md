...
| Verified | 2026-04-29 |
| Tags | project-flow, operations, workflow, automation, pipeline |

### Description

项目流运营自动化——管理和优化跨团队的工程项目流水线。

### How It Works

project-flow-ops Skill 让 Agent 管理项目开发流水线的全生命周期：PR 审批流、CI/CD 状态监控、部署编排、团队工作负载分配。自动识别瓶颈环节（积压的 PR、频繁失败的构建），并生成流程优化建议。

### Quick Start

1. 安装 ECC 的 project-flow-ops Skill
2. 连接 GitHub/GitLab 仓库和 CI/CD 系统
3. Agent 自动分析项目流水线状态
4. 输出瓶颈报告和流程优化建议
